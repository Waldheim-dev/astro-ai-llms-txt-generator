...
/**
 * Returns the main summary prompt.
 * @param language - The language for the prompt.
 * @returns The prompt string.
 */
export function getMainSummaryPrompt(language: string = 'en'): string {
  const base =
    "Act as an expert technical writer. Create a high-quality Markdown H1 and a blockquote for an 'llms.txt' file. " +
...
/**
 * Returns the details section prompt.
 * @param language - The language for the prompt.
 * @returns The prompt string.
 */
export function getDetailsPrompt(language: string = 'en'): string {
  const base =
    "Provide a detailed but concise Markdown section (no heading) for an 'llms.txt' file. " +
...
export function getFileListPrompt(language: string = 'en', section: string = 'Docs'): string {
  const base =
    `Create a Markdown H2 named '${section}' followed by a list of links. ` +
...
/**
 * Returns the full llms.txt prompt.
 * @param language - The language for the prompt.
 * @returns The prompt string.
 */
export function getFullLlmsTxtPrompt(language: string = 'en'): string {
  const base =
    "Generate a complete 'llms.txt' following the standard. Include an H1 title, a blockquote summary, " +
